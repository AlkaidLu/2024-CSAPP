...
def cycle():
    ...
    if opcode==0: #halt
        ...
        #set CC
            ...
            if(fn==0): #jmp
                pReg=Dest
            elif(fn==1): #jle(jump if less than or equal)
                if(SF^OF)|ZF:
                    pReg=Dest
            elif(fn==2): #jl(jump if less than)
                if SF^OF:
                    pReg=Dest
            elif(fn==3): #je(jump if equal)
                if ZF:
                    pReg=Dest
            elif(fn==4): #jne(jump if not equal)
                if ZF==0:
                    pReg=Dest
            elif(fn==5): #jge(jump if greater than or equal)
                if (SF^OF)==0:#if (~(ZF^OF))|ZF:
                    pReg=Dest
            elif(fn==6): #jg(jump if greater than)
                if ((SF^OF)==0)&(ZF==0):#if ~(ZF^OF):
                    pReg=Dest
        elif opcode == 8:    #call
            Dest = int(get_Dest(instr),16)
            reg[4] = reg[4] - 8
