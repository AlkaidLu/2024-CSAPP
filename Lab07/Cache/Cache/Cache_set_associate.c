	...
	*/
	...
	if(Found==0)
	{
        ...
        if (LineOffset == DCACHE_LINE_PER_SET)
		{
			...
			OldAddress = ((DCache[CacheLineAddress].Tag << DCACHE_SET_ADDR_BITS) << DCACHE_DATA_PER_LINE_ADDR_BITS) | ((UINT64)CacheSetIndex << DCACHE_DATA_PER_LINE_ADDR_BITS);	// 从Tag中恢复旧的地址
