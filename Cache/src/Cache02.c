        ...
        for(i=0;i<VCACHE_LINE;i++){
            if(VCache[i].Valid==1&& VCache[i].Tag==AddressTag)
            {
                ...
                if (LineOffset == DCACHE_LINE_PER_SET)
                {
                    ...
                    Address = ((DCache[CacheLineAddress].Tag << DCACHE_SET_ADDR_BITS) << DCACHE_DATA_PER_LINE_ADDR_BITS) | ((UINT64)(CacheSetIndex*DCACHE_LINE_PER_SET) << DCACHE_DATA_PER_LINE_ADDR_BITS);	// ��Tag�лָ��ɵĵ�ַ
