...
#define L2CACHE_DATA_PER_LINE           16
...
}DCache[DCACHE_SET*DCACHE_LINE_PER_SET],VCache[VCACHE_LINE],ICache[ICACHE_LINE],L2Cache[L2CACHE_LINE];
//DCache组的结构
...
*/
void StoreDataCacheLineToVictimCache(UINT32 VCacheLineAddress, UINT32 DCacheLineAddress)
{
    VCache[VCacheLineAddress].Data= DCache[DCacheLineAddress].Data;
	...
	*/
    ...
    if(Found==0){
        ...
        for(i=0;i<VCACHE_LINE;i++){
            if(VCache[i].Valid==1&& DCache[i].Tag==AddressTag)
            {
                ...
                if (LineOffset == DCACHE_LINE_PER_SET)
                {
                    ...
                    UINT64 VictimAddress;
                    // OldAddress = > (Tag,Set,0000)
                    VictimAddress = ((DCache[CacheLineAddress].Tag << DCACHE_SET_ADDR_BITS) << DCACHE_DATA_PER_LINE_ADDR_BITS) | ((UINT64)CacheLineAddress << DCACHE_DATA_PER_LINE_ADDR_BITS);	// 从Tag中恢复旧的地址
                    StoreDataCacheLineToVictimCache(VictimAddress, CacheLineAddress);
                }
                // 需要从VictimCache中读入新的行（真实情况下，这个LoadCacheLineFromMemory需要很长时间的）
                LoadDataCacheLineFromVictim(Address, CacheLineAddress);
