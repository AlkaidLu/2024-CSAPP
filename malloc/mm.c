...
int mm_init(void)
{
    ...
    if ((heap_listp = mem_sbrk((4+LIST_MAX-4) * WSIZE)) == (void *)-1) // line:vm:mm:begininit
        return -1;
    int i;
...
static void *find_fit(size_t asize) {
    ...
    for (int i = num; i <= LIST_MAX; i++) {
        if (GET_HEAD(i)) {
            bp = GET_HEAD(i);
             printf("find_fit: found a block of size %zu in list %d\n", asize, num);
            // 在这里可以对bp进行进一步处理，比如检查bp指向的内存块是否满足条件
